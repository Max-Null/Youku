...
(function(){
		...
		$.ajax({
			...
			success:function(data) { 
				if(categoryCallback != ''){
						...
						eval("window."+categoryCallback+"(window.categoryData)");	
			   }else{
					if(data.categories) {
					   var tpl = '';
    ...
    var cancelUploadTask = function (uploadToken){
		...
		$.ajax({
            type: 'POST',
            url: uploadOptions["api_url"] + "v2/uploads/web/cancel.json",
            data: params,
			dataType: "JSONP", 
            success:function(data) { 
				if(browserJudge() == "chrome"){
    ...
    var createUploadTask = function (){
        ...
        $.ajax({
            ...
            success:function(data) { 
                ...
                if (uploadToken) {
                    var tpl = '<h1>正在上传视频</h1><p>请不要关闭浏览器，此操作会造成上传失败!';
                    tpl += '<br>上传需要一段时间，请耐心等待.</p><br><div class="progress progress-success" style="margin-bottom: 9px;">';
                    tpl += '<div class="bar" style="width: 0%"></div></div><div class="progress-extended" style="margin-bottom: 20px;">';
                    tpl +=  '<span id="tdCurrentSpeed">00.00 kbit/s</span> | <span id="tdTimeRemaining">00:00:00</span> | <span id="tdPercentUploaded">00.00 %</span> | <span id="tdSizeUploaded">00.00 KB</span> / <span id="bytestotal">00.00 KB</span></div>';
                    tpl += '<div style="text-align: center; margin-bottom: 15px;"><button type="button" class="btn btn-primary start" id="btn-upload-stop"><span>取消上传</span></button></div>';
	...
	function loadSWFUpload(){
		settings_object = {
			...
			flash_url : "http://open.youku.com/assets/lib/swfupload/swfupload2.swf",
			
			// Button settings
			button_image_url: "http://open.youku.com/assets/lib/images/XPButtonUploadText_61x22x4.png",
			button_width: "63",
			button_height: "22",
